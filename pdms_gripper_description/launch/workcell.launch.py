...
def generate_launch_description():
    ...
    objects_arg = DeclareLaunchArgument(
        name="objects", default_value="none",
        description="object manifest in config/, e.g. objects_rail.yaml for the 3-station rail")
    ...
    return LaunchDescription([
        ...
        ExecuteProcess(
            cmd=["python3", os.path.join(pkg, "tools", "pipette_attach.py"),
                 "--ros-args", "-p", ["objects:=", LaunchConfiguration("objects")]],
            name="pipette_attach",
            output="screen",
        ),
        ExecuteProcess(
